...
export interface CERPriceResponse {
  price: number;
  timestamp: Date;
  currency: string;
  change24h?: number;
...
/**
 * Generates realistic fallback CER price based on market trends
 * CER typically trades at 30-50% discount to EUA prices
 * Used when backend API is unavailable
 */
function generateRealisticPrice(): CERPriceResponse {
  ...
  // Simulate realistic price movement (CER typically moves 1-3% per day)
...
/**
 * Fetches the latest Chinese CER carbon price from Python backend API
 * Falls back to realistic simulation if backend is unavailable
 * @returns Promise with price data
 */
export async function fetchCERPrice(): Promise<CERPriceResponse | null> {
  try {
    // Call Python backend API
    const apiPath = BACKEND_API_URL.startsWith('/') 
      ? `${BACKEND_API_URL}/cer/price` 
      : `${BACKEND_API_URL}/api/cer/price`;
    ...
    if (response.data && response.data.price) {
      const priceData: CERPriceResponse = {
    ...
    console.warn('Error fetching CER price from backend, using fallback:', error);
...
/**
 * Fetches CER price with retry logic
 * @param maxRetries Maximum number of retry attempts
 * @param retryDelay Delay between retries in milliseconds
 * @returns Promise with price data or null if all retries fail
 */
export async function fetchCERPriceWithRetry(
  maxRetries: number = 3,
  retryDelay: number = 2000
): Promise<CERPriceResponse | null> {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    const result = await fetchCERPrice();
...
/**
 * Fetches historical CER price data from backend API
 ...
 */
export async function fetchCERHistory(
  ...
  try {
    ...
    const apiPath = BACKEND_API_URL.startsWith('/') 
      ? `${BACKEND_API_URL}/cer/history` 
      : `${BACKEND_API_URL}/api/cer/history`;
    ...
    console.warn('Error fetching CER history from backend:', error);
    return null;
  }
}
