# Create Project Brief

Please create a product brief based on the description that the user provides. Your goal is to capture the business and functional requirements of the product and to provide solid context for others working on the product. You should include:

1. Project overview / description
2. Target audience
3. Primary benefits / features
4. High-level tech/architecture used

Keep the brief very concise and to the point just to give enough context to understand the bigger picture.

## Outputs

1. **Product Brief**: Write the document into `docs/PRODUCT_BRIEF.md` (unless a different file name is specified).

2. **Application Truth**: Create `app-truth.md` at the project root with the following information:
   - Architecture patterns and system design
   - Communication protocols between components
   - Port configurations and endpoints
   - Database schemas and conventions
   - Naming conventions and coding standards
   - API contracts and response formats
   - Security requirements and authentication patterns
   - Performance parameters and requirements
   - Environment variables and configuration
   - Deployment and infrastructure setup

The `app-truth.md` file serves as the single source of truth for technical specifications and will be referenced by all other development commands (planning, implementation, review, documentation) to ensure consistency.

